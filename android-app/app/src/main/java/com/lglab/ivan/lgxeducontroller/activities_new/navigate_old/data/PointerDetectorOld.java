package com.lglab.ivan.lgxeducontroller.activities_new.navigate_old.data;

public class PointerDetectorOld {
    ...
    public PointerDetectorOld(float x, float y) {
        xAfter = -1;
        yAfter = -1;
        update(x, y);
    }
    ...
    private double getDistanceFromPointerAfter(PointerDetectorOld pointer) {
        return Math.sqrt(Math.pow(xAfter - pointer.xAfter, 2) + Math.pow(yAfter - pointer.yAfter, 2));
    }

    private double getDistanceFromPointerBefore(PointerDetectorOld pointer) {
        return Math.sqrt(Math.pow(xBefore - pointer.xBefore, 2) + Math.pow(yBefore - pointer.yBefore, 2));
    }

    public short getZoomInteractionType(PointerDetectorOld pointer) {

...
public class ExpandableList {
  ...
  public void remove(int groupIndex) {
    ...
    boolean[] expandedGroupIndexesTemp = expandedGroupIndexes.clone();
    expandedGroupIndexes = new boolean[groups.size()];
    for (int i = 0; i < groups.size(); i++) {
      if (i < groupIndex) {
        expandedGroupIndexes[i] = expandedGroupIndexesTemp[i];
      } else {
        expandedGroupIndexes[i] = expandedGroupIndexesTemp[i + 1];
      }
    }
  }
}

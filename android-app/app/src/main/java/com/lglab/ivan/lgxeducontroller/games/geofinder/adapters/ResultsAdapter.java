...
public class ResultsAdapter extends RecyclerView.Adapter<ResultsAdapter.ResultsViewHolder> {
    ...
    public void onBindViewHolder(ResultsViewHolder personViewHolder, int i) {
        GeoFinderQuestion question = (GeoFinderQuestion) GameManager.getInstance().getGame().getQuestions().get(i);

        personViewHolder.questionName.setText(question.getQuestion());
        personViewHolder.questionNumber.setText(String.valueOf(i + 1));
        personViewHolder.questionSolution.setText("Score points: " + ((GeoFinderManager)GameManager.getInstance()).getScoreQuestion(i));
